...
<head>
    ...
    <title>8068 CAR SQUARE | POS System v7.2</title>
    ...
    <style>
        * { font-family: 'Inter', sans-serif; }
        ...
        .commission-total { font-size: 24px; font-weight: bold; }
        ...
        .staff-card { transition: all 0.3s; }
        .staff-card:hover { transform: translateY(-4px); box-shadow: 0 8px 30px rgba(0,212,255,0.15); }
    </style>
</head>
...
<script>
    ...
    const saved = localStorage.getItem("8068_enterprise_v7_2");
    if(saved) {
        try {
    ...
    function saveToLocal() {
        localStorage.setItem("8068_enterprise_v7_2", JSON.stringify(appData));
        ...
        return `<div class="min-h-screen flex items-center justify-center">
            <div class="card-premium p-8 max-w-md w-full">
                <div class="text-center mb-6">
                    ...
                    <p class="text-cyan-400 text-xs mt-2">Enterprise POS System v7.2</p>
                </div>
                <div class="space-y-4">
    ...
    function exportToExcel(reportType) {
        ...
        if (reportType === 'daily') {
            ...
            data = [
                ...
                [''],
                ['GRAND TOTAL', '', '', '', '', '', `₱${totalCommission.toFixed(2)}`]
            ];
    ...
    function renderReports() {
        ...
        const commissionReport = generateCommissionReport();
        ...
        return `<div class="card-premium p-6">
            ...
            <div class="bg-gray-800 p-4 rounded-lg">
                <div class="flex justify-between items-center mb-3">
                    <h3 class="text-lg font-bold text-purple-400"><i class="fas fa-file-invoice-dollar"></i> Employee Commission Report</h3>
                    <button onclick="exportToExcel('commission')" class="excel-export-btn px-4 py-2 rounded text-sm text-white"><i class="fas fa-file-excel"></i> Export Commission Excel</button>
                </div>
                <div class="overflow-x-auto max-h-64 overflow-y-auto">
                    <table class="w-full text-sm">
                        <thead class="sticky top-0 bg-gray-900 border-b border-purple-400">
                            <tr><th>Staff Name</th><th>Position</th><th>Tier</th><th>Jobs</th><th>Sales</th><th>Daily Commission</th><th>Total Commission</th></tr>
                        </thead>
                        <tbody>
                            ${commissionReport.map(s => `
                                <tr class="border-b border-gray-700">
                                    ...
                                    <td class="text-cyan-400">₱${s.dailyCommission.toFixed(2)}</td>
                            ...
                            <tr class="border-t-2 border-purple-400 font-bold">
                                ...
                                <td class="text-green-400">₱${commissionReport.reduce((sum, s) => sum + s.totalCommission, 0).toFixed(2)}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    // ======================== DASHBOARD WITH DAILY COMMISSIONS ========================
    function renderDashboard() {
        ...
        // Commission feed - recent commission transactions
    ...
    window.saveAsOpenService = () => {
        ...
        const openService = createOpenService(customer, vehicle, plate, posCart, staff, total, cartRemarks);
    ...
    function renderAdminSystem() {
        ...
        return `<h3 class="text-xl font-bold mb-4"><i class="fas fa-cogs"></i> System Settings</h3>
            ...
            <div class="mt-4 p-3 bg-gray-800 rounded-lg text-xs">
                ...
                <div>Version: 7.2 Enterprise | Last Saved: ${localStorage.getItem('8068_system_last_saved') ? new Date(localStorage.getItem('8068_system_last_saved')).toLocaleString() : 'Never'}</div>
        ...
        <div class="max-w-screen-2xl mx-auto px-4 py-5">
            <div class="flex flex-wrap justify-between items-center border-b border-blue-800 pb-4 mb-6">
                <div class="flex items-center gap-3">
                    ...
                    <div><h1 class="text-3xl font-extrabold glow-text">${appData.systemSettings.companyName}</h1><p class="text-sm text-gray-400">${appData.systemSettings.tagline} | Enterprise v7.2</p></div>
                </div>
    ...
    console.log('%c8068 CAR SQUARE - Enterprise Management System v7.2', 'color: #00d4ff; font-size: 20px; font-weight: bold;');
    ...
    console.log(`%cDaily Commissions: ${appData.staff.reduce((sum, s) => sum + (s.totalCommission || 0), 0).toFixed(2)}`, 'color: #ffaa00;');
</script>
</body>
</html>